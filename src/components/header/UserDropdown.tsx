...
export default function UserDropdown() {
  ...
  const { data: dataProfiles, isError: errorFetchProfiles } =
    useReadProfiles(activeFilter);

  const profiles = dataProfiles && dataProfiles?.data?.profile;

  useEffect(() => {
    if (errorFetchProfiles || !profiles) {
      Cookies.remove('token');
    }
  }, [errorFetchProfiles, profiles]);
